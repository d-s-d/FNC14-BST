...
static inline size_t compute_size_align( size_t n, size_t al ) {
    size_t m = (n+1)%al;
    size_t k = ((al-1)-m)*((al-1)-m+1)/2;
    size_t p = (al-1)*al/2;
    size_t rempad = p - k;
    return (n+1)*(n+2)/2 + ((n+1)/(al))*p + rempad;
}

void* bst_alloc_128_m128_aligned4_root_aligned( size_t n ) {
    segments_t* mem = (segments_t*) malloc( sizeof(segments_t) );
    mem->e_sz = (n+1)*(n+2)/2 + n/2 + 1;
    mem->r_sz = compute_size_align( n, 4 );
    // n/2 + 1 is the padding space
    // XXX: for testing: calloc
    mem->e = (double*) _mm_malloc(mem->e_sz* sizeof(double), 16 );
    mem->w = (double*) _mm_malloc(mem->e_sz* sizeof(double), 16 );
    mem->r = (int*)    _mm_malloc(mem->r_sz* sizeof(int),    16 );
    mem->n = n;
    memset( mem->r, -1, mem->r_sz * sizeof(int) );
    return mem;
}

double bst_compute_128_m128_aligned4_root_aligned( void*_bst_obj, double* p, double* q, size_t nn ) {
    ...
    for (i = n-1; i >= 0; --i) {
        ...
        for (r = i; r < n; ++r) {
            ...
            e_tmp     = e[idx1++];
            idx1_root++;
            // calculate until a multiple of 4 doubles is left
            ...
            // execute the shit for 2 vectors of size 2
            ...
            for( ; idx2 < l_end; idx2 += 4 ) {
                ...
                */
                _mm_maskstore_ps( &root[idx1_root],
                        _mm_castps_si128( v_rootmask  ),
...
size_t bst_get_root_128_m128_aligned4_root_aligned( void* _bst_obj, size_t i, size_t j )
{
...
void bst_free_128_m128_aligned4_root_aligned( void* _mem ) {
...
size_t bst_flops_128_m128_aligned4_root_aligned( size_t n ) {
    double n3 = n*n*n;
    double n2 = n*n;
