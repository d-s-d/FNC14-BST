...
/*
 * In this optimization, we simply use shorts instead of ints to store the
 * root-table.
 */
...
void* bst_alloc_004_short_root( size_t n ) {
    ...
    mem->r = (short*)  malloc( sz2 * sizeof(short) );
    memset( mem->r, -1, sz2 * sizeof(short) );
    return mem;
}

double bst_compute_004_short_root( void*_bst_obj, double* p, double* q,
                                   size_t n ) {
...
size_t bst_get_root_004_short_root( void* _bst_obj, size_t i, size_t j )
{
...
void bst_free_004_short_root( void* _mem ) {
...
size_t bst_flops_004_short_root( size_t n ) {
    size_t n3 = n*n*n;
    size_t n2 = n*n;
